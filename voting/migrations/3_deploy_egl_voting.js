...
const mockBalancerPoolTokenSupply = web3.utils.toWei(
    '5693913.291241588098514244'
)
...
module.exports = async function(deployer, network, accounts) {
    ...
    if (network === 'ropsten') {
        const MockBalancerPoolToken = artifacts.require(
            './helpers/MockBalancerPoolToken.sol'
        )
        ...
        creatorRewardAccount = '0xCA78C111CF45FE0B8D4F3918632DDc33917Af882'
    }
    ...
    if (network === 'ganache') {
        ...
        await mockEglGenesis.sendTransaction({
            from: accounts[8],
            value: web3.utils.toWei('0.01'),
        })
        console.log(
            `Contributed 0.1 ETH to Genesis from account: ${ConsoleColors.YELLOW}`,
            accounts[8]
        )
        ...
        eglOwner = accounts[1]
        eglTokenAddress = eglToken.address
        eglGenesisAddress = '0x6B788EF7dED8236034d019F5fa3CB96E6017f618'
        balancerPoolTokenAddress = mockBalancerPoolToken.address
        firstEpochStartDate = await getBlockTimestamp(web3)
        votePauseSeconds = 1 // 1 seconds
    ...
    console.log(
        `Using EGL Token at address: ${ConsoleColors.CYAN}`,
        eglTokenAddress
    )
    ...
    console.log(
        `EGL Contract ownership transferred to account: ${ConsoleColors.YELLOW}`,
        eglOwner
    )
}
